...
% DirectionQuote: -1 for XXX/USD /\ 1 for USD/XXX
% see spreadsheet FX_NDF_Built_and_PLMethodo.xls
%
%__________________________________________________________________________
%

function [grossreturn_i , tcforec_i] = computeOpenFwdBasedReturn(i, c, p, pfwd, s, wgt, TC)
...
% Adjust transaction cost
    ...
    if ~isnan(c(i,j)) && ~isnan(c(i-1,j)) && c(i,j)~=0 && c(i-1,j)~=0 && p(i-1,j)>0 && p(i,j)>0
        grossreturn_i(1,j) =  s(i-1,j) * wgt(i-1,j) * (log(p(i,j))-log(pfwd(i-1,j)));
