function[eqprice] = NonLinearReg(x,Lookback,method)
%
...
%
% -- Prelocate Matrix --
[nsteps,ncols]=size(x);     intercept = zeros(size(x));
slope = zeros(size(x));     resid = zeros(size(x));
eqprice = zeros(size(x));
%
% -- Set Time vector --
...
for j=1:ncols
    ...
    for i=start_date(1,1)+Lookback:nsteps
        ...
        [b, ~ , r] = regress(NormMyX , [ct , TimeTrend', log(TimeTrend')]); 
        intercept(i,j) = b(1,1);
        slope(i,j) = b(2,1);
        resid(i,j) = r(length(r),1);
        eqprice(i,j) = [1 , Lookback, log(Lookback)] * b;        
    end
end
