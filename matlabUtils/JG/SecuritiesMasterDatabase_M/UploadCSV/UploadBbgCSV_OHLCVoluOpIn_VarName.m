function [datev, dateBbg, o, h, l, c, volu, opint] = UploadBbgCSV_OHLCVoluOpIn_VarName(path , VarName)
%
...
OptionRead = 2;
if OptionRead == 1
    data = csvread(strcat(path,VarName,'.txt'));
elseif OptionRead == 2
    data = dlmread(strcat(path,VarName,'.txt'));%, 'delimiter', ',',);
end
%
