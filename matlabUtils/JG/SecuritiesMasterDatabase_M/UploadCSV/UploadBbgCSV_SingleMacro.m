function [datev, dateBbg, c] = UploadBbgCSV_SingleMacro(path , AssetKey)
%
...
OptionRead = 2;
if OptionRead == 1
    data = csvread(strcat(path,sprintf('macro%d.mat', AssetKey)));
elseif OptionRead == 2
    data = dlmread(strcat(path,sprintf('macro%d.txt', AssetKey)));%, 'delimiter', ',',);
end
%
