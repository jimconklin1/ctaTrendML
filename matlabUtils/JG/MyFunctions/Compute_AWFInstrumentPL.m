...
function [grossreturn_i , tcforec_i] = Compute_AWFInstrumentPL(i, j, c, p, tradables, ExecP, s, wgt, nb, TC)
%
...
if tradables(1,j)==1 && s(i,j)~=s(i-1,j) || (s(i,j)==s(i-1,j) && nb(i,j)==nb(i-1,j))
    Ftc3=0;
...
if tradables(1,j)==1 && s(i,j) ~= s(i-1,j) || (s(i,j) == s(i-1,j) && wgt(i,j) == wgt(i-1,j)) 
    Ftc3wgt = 0;
...
if tradables(1,j)==1 && ~isnan(p(i,j)) && ~isnan(p(i-1,j)) && p(i,j)~=0 && p(i-1,j)~=0
