function portStruct = calcDDandPnL2(configPortfolio, dataSet, portStruct)

% NOTE: version 2, unlike calcDDandPnl(), does not use the portfolio
%   structures of the suite of trend models.  Rather, 

T = size(dataSet.dates,1); 
if ~isfield(portStruct,'assetIndx') && isfield(portStruct,'indx')
   nn = portStruct.indx; 
else    
...
%        pnl(t,:) = nansum([pnlPos(t,:); pnlTrdg(t,:); pnlTC(t,:); pnlBrrw(t,:)],1); 
    ...
    for t = t1:T
        ...
        if ismember(dataSet.dates(t),dataSet.holidays.datenum_holiday)
            ddWts(t,:) = ddWts(t-1,:); 
...
% Convention 1 for "bringing weights forward to trade day":
% portStruct.tradeDates = busdate(portStruct.dates,1);
% portStruct.ddrawdown = Ddrawdown;
% portStruct.rawRtns = rtnCl; 
% Convention 2:
portStruct.dates = busdate(portStruct.dates,1); 
portStruct.tradeDates = busdate(portStruct.dates,1); 
...
portStruct.pnl = pnl(2:end,:); 
portStruct.pnl(T,:)=0; 
portStruct.pnlTot = pnlTot(2:end,:); 
portStruct.pnlTot(T,:)=0; 
portStruct.cumPnl = cumPnl(2:end,:); 
portStruct.cumPnl(T,:) = portStruct.cumPnl(T-1,:);  
