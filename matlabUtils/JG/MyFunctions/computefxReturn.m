...
% Three different solutions are suggested
...
function [grossreturn_i , tcforec_i] = computefxReturn(method, i, c, p, fwdTom, s, wgt, loc_rate, usd_rate, DirectionQuote, FwdPointQuotationFactor, TC, yrNbDays)
    ...
    case {'method2', 'm2'}
        ...
        for j=1:ncols   
            ...
            if ~isnan(c(i,j)) && ~isnan(c(i-1,j)) && c(i,j)~=0 && c(i-1,j)~=0
                ...
                % legacy : DirectionQuote - -1 for XXX/USD /\ 1 for USD/XXX
                grossreturn_i(1,j) = grossreturn_i(1,j) + ...
                                   s(i-1,j) * wgt(i-1,j) * DirectionQuote(1,j) * (USRateAdjustment * usd_rate(i-1) - LocRateAdjustment * loc_rate(i-1,j)) / yrNbDays;    
    ...
    case {'method3', 'm3'}
        ...
        for j=1:ncols   
            ...
            if ~isnan(c(i,j)) && ~isnan(c(i-1,j)) && c(i,j)~=0 && c(i-1,j)~=0
                ...
                % add carry
                if s(i-1,j)==1
                    fwdAdjustment = 1; % penalize lending, increase borrow
                else
                    fwdAdjustment = 1; % penalize lending, increase borrow               
                end
                % difference in rate from forward points
                carryReturn = - DirectionQuote(1,j) * s(i-1,j) * (fwdTom(i,j)/FwdPointQuotationFactor)/yrNbDays;
                % legacy : DirectionQuote : -1 for XXX/USD /\ 1 for USD/XXX
                grossreturn_i(1,j) = grossreturn_i(1,j) + wgt(i-1,j) * carryReturn;    
                tcforec_i = Ftc1*TC(1,j)*wgt(i-2,j) + Ftc2*TC(1,j)*wgt(i-1,j) + Ftc3*TC(1,j)*abs(wgt(i-1,j)-wgt(i-2,j)); % wgt or nb
            else
                grossreturn_i(1,j) = 0; 
                tcforec_i(1,j) = 0;
