function [y, alpha_out, b_out] = svReg(xdata,ydata,x,alpha, b, c, epsilon)
...
%  Function call:
%
%    [y, alpha0,b0] = SVReg(xdata,ydata,x,alpha,b,c,epsilon);
%    [~,alpha0,b0] = SVReg(xdata,ydata,[],[],[],c,epsilon);
%    y = SVReg(xdata,ydata,x,[],[],[],[]);
%    y = SVReg(xdata,[],x,alpha,b,[],[]);
%
%  Example usage:
%
...
function svr_train()
    ...
    M = M + 1/c*eye(ntrain);
    image(M)    
    % Matlab version 2012b and after
    %options = optimoptions('fmincon','Algorithm','interior-point','MaxFunEvals',100000,'TolX',1e-3,'Display','iter');
    options = optimset('Algorithm','interior-point','MaxFunEvals',400,'TolX',1e-3,'Display','iter');
