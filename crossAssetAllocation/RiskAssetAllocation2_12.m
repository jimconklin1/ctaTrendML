...
%
%
function RiskAssetAllocation2_12(assetView,filePath,rebalPeriods,utilityFunction,utilityParam,shrinkageRP,...
...
elseif assetView == "AIG"
    ...
    switch riskType 
        case 'Accounting'
           ...
           factorERset =[0.070124572,  0.011800061, 0.012945044, 0.013452167]; %#ok<NASGU> % annual units here
           factorVolSet =[0.174271928, 0.071595505,0.065505117,0.067080039];
           keySet = {'PvtEqBO','PvtEqRE','PvtEqFT','HFBeta0','ARP',  'AbsRtn','MSR'};
           erSet = [0.0991,  0.0682,   0.08688,    0.0486,   0.06684,  0.0735, 0.11];
           volSet = [0.1292, 0.076,    0.0811,     0.06,     0.0887,   0.1,    0.15];
%            keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HYcorp','HYem','PvtCrMM','HFBeta','HFBeta0','ARP','AbsRtn'};
...
%                     PE      GRE     FT 0beta ARP absRtn MSR
        curWeights = [0.13688,0.2346, 0, 0,    0,  0,     0]; % reduced assets
        curWeights = curWeights/sum(curWeights); % reduced assets
    else
        curWeights = [0.010713,0.13688,0,0.2346,0.05225,0.014875,0.32574,0.224923,0,0,0];
    end
end
