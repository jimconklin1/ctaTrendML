...
function [grossreturn_i , tcforec_i] = computeOpenRateBasedReturn2(method, i, c, p, s, wgt, loc_rate, usd_rate, DirectionQuote,TC, yrNbDays)
    ...
    case {'method2', 'm2'}
        ...
        for j=1:ncols   
            ...
            if ~isnan(c(i,j)) && ~isnan(c(i-1,j)) && c(i,j)~=0 && c(i-1,j)~=0
                ...
                % legacy : DirectionQuote - -1 for XXX/USD /\ 1 for USD/XXX
                grossreturn_i(1,j) = grossreturn_i(1,j) + ...
                                     s(i-1,j) * wgt(i-1,j) * DirectionQuote(1,j) * ...
                                     (USRateAdjustment * usd_rate(i-1,j) - LocRateAdjustment * loc_rate(i-1,j)) / yrNbDays;    
