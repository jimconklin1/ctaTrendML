function bmat = RollingLogitXY(x, y, LagDiff, LagStructure, Lookback_Period)
...
for i = Lookback_Period + LagStructure : nsteps
    ...
    ydiff_snap = ydiff(i-Lookback_Period+1:i);
    try
        b = glmfit(x_snap, ydiff_snap, 'binomial');
    %catch
        if size(b,1)== ncols+1;
            bmat(i,:)=b';
        else
