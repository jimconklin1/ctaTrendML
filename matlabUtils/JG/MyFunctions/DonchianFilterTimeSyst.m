function[s, sumprofit, sumgrossprofit]  =  DonchianFilterTimeSyst(o, h, l, c, Lookback, maf, mas, MaxHPShort, MaxHPLong)
%
%__________________________________________________________________________
%
% DONNCHIAN WITH MOVING AVERAGE FILTER AND TIME-BASED EXIT SYSTEM
%
%__________________________________________________________________________
%
%__________________________________________________________________________
...
MaxHighEnterLong=zeros(size(c));  MinLowExitLong=zeros(size(c));
MinLowEnterShort=zeros(size(c));  MaxHighExitShort=zeros(size(c));
for i=Lookback(1,1):nsteps
    MaxHighEnterLong(i)=max(h(i-Lookback(1,1)+1:i,1));
end
for i=Lookback(1,2):nsteps
    MinLowExitLong(i)=min(l(i-Lookback(1,2)+1:i,1));
end
for i=Lookback(1,3):nsteps
    MinLowEnterShort(i)=min(l(i-Lookback(1,3)+1:i,1));
end
for i=Lookback(1,4):nsteps
    MaxHighExitShort(i)=max(h(i-Lookback(1,4)+1:i,1));
end
%
...
for i=144:nsteps   
    ...
    % No extra condition...................................................   
    % Enter Short Stock------------------------------------------------
    if  s(i-1)~=-1 && c(i)<MinLowEnterShort(i-1)  && maf(i)<mas(i)  
        % Signal
        s(i)=-1;  
        ...
        HoldShort(i)=0;         
    % Hold Short Stock.................................................
    elseif s(i-1)==-1 && c(i)<MaxHighExitShort(i-1) && maf(i)<mas(i) && HoldShort(i-1)<MaxHPShort 
        % Keep Signal
        s(i)=-1;  
        % Keep nb of shares
        nb(i)=nb(i-1);
    ...
    % Enter Long Stock-------------------------------------------------
    elseif s(i-1)~=1 && c(i)>MaxHighEnterLong(i-1)  && maf(i)>mas(i) 
        % Signal
        s(i)=+1;  
        % Compute Number of Shares
        nb(i)=1;%capital/p(i);          
        ...
        HoldLong(i)=0;
    % Hold Long Stock...................................................
    elseif s(i-1)==1 && c(i)>MinLowExitLong(i-1) && maf(i)>mas(i) &&  HoldLong(i-1)<MaxHPLong
        % Keep Signal
        s(i)=+1;
        % Keep nb of shares
        nb(i)=nb(i-1);          
