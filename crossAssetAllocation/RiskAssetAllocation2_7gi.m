...
%
%
function RiskAssetAllocation2_7gi(assetView,filePath,rebalPeriods,utilityFunction,utilityParam,shrinkageRP,...
...
keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HFBeta','HFBeta0','ARP','AbsRtn'};
...
if assetView == "JPM"
   ...
   keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HFBeta','HFBeta0','ARP','AbsRtn'};
   erSet = [0.0707	0.1059	0.0675	0.0777	0.045	0.057	0.0709	0.083];
   volSet = [0.149	0.2017	0.14125	0.1382	0.0737	0.06	0.1308	0.1];
else 
   switch riskType 
        case 'Accounting'
           factorKeySet = {'Equity','Rates','Kredit','Mtg'};
           ...
           factorVolSet =[0.174271928, 0.071595505,0.065505117,0.067080039];
           keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HFBeta','HFBeta0','ARP','AbsRtn'};
           erSet =  [0.0614,        0.0991,    0.0682,   0.08688,  0.0433,  0.0486,   0.06684,  0.0735];
           volSet = [0.151,         0.1292,    0.076,    0.0811,   0.076,   0.06,     0.0887,   0.1];
%            keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HYcorp','HYem','PvtCrMM','HFBeta','HFBeta0','ARP','AbsRtn'};
...
%            volSet = [0.151325,    0.1292,     0.076,    0.0811,   0.07378, 0.0781, 0.11253, 0.07574, 0.06,   0.0887,  0.1];
        case 'Intrinsic'
           ...
           keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HFBeta','HFBeta0','ARP',   'AbsRtn'};
           erSet =  [0.0614,        0.1003,    0.0644,   0.08688,  0.0433,  0.0486,   0.06684, 0.0735];
           volSet = [0.151,         0.207,     0.153,    0.114,    0.076,   0.06,     0.0887,  0.1];
%            keySet = {'GlobalEquity','PvtEqBO','PvtEqRE','PvtEqFT','HYcorp','HYem','PvtCrMM','HFBeta','HFBeta0','ARP','AbsRtn'};
...
if assetView ~= "Historical" && assetView ~= "AIG" && assetView ~= "JPM"
...
curWeights =[];
if client == "L&R"
   curWeights = [0.019631,0.0804916,0.117991,0,0.052974,0,0,0];
   curWeights = curWeights/sum(curWeights); 
elseif client == "GI"
   curWeights = [0.010713,0.13688,0,  0.2346,0.224923,0,0,0];
   curWeights = curWeights/sum(curWeights); 
end
...
if strcmpi(assetView,'JPM')
   title(['Portfolio Analysis ',client,', JPM LTCMA'])
else
