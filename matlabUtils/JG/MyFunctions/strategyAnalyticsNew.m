...
function [analyticsT, outan] = strategyAnalyticsNew(datesStamp, s,wgt, ptfec, assetReturns, holdingPeriodLong, holdingPeriodShort, strategyStyle, doChart)
...
if strcmp(strategyStyle,'TimeSeries') || strcmp(strategyStyle,'timeSeries') || strcmp(strategyStyle,'ts')
    ...
    % Average holding period Long        
    hpLongInOut = zeros(1,1);
    retLongInOut = zeros(1,1);
    Cumsum_assetReturns = cumsum(assetReturns(:,1));
    for i=startRow-1:nsteps
        if (s(i) == 0 || s(i) == -1) && s(i-1) == 1
            hpLongInOut =  [hpLongInOut  ; holdingPeriodLong(i-1,1)];
            retLongInOut = [retLongInOut ; Cumsum_assetReturns(i,1)];
        end
    end
    hpLongInOut(1,:)=[];
    retLongInOut(1,:)=[];
    avghpLong = mean(hpLongInOut);
    avgretLong = mean(Delta(retLongInOut,'d',1));
    outan.avghpLong = floor(avghpLong);
    if outan.avghpLong == 0 , outan.avghpLong = 1; end
    outan.avgretLong = avgretLong;

    % Average holding period Short        
    hpShortInOut = zeros(1,1);
    retShortInOut = zeros(1,1);
    for i=startRow-1:nsteps
        if  (s(i) == 0 || s(i) == 1) && s(i-1) == -1
            hpShortInOut = [hpShortInOut ; holdingPeriodShort(i-1,1)];
            retShortInOut = [retShortInOut ; Cumsum_assetReturns(i,1)];
        end
    end
    hpShortInOut(1,:)=[];
    retShortInOut(1,:)=[];
    avghpShort = mean(hpShortInOut);
    avgretShort = mean(Delta(retShortInOut,'d',1));   
    outan.avghpShort = floor(avghpShort);
    if outan.avghpShort == 0 , outan.avghpShort = 1; end
    outan.avgretShort = avgretShort;
...
elseif strcmp(strategyStyle,'allocationModel')    
...
totPerReturnsClean = unique(totPerReturns);% first clean returns for days without signals
totPerIr = 16 * mean(totPerReturnsClean) / std(totPerReturnsClean);
totPerSR = 16 * mean(totPerReturnsClean) / std(totPerReturnsNegative);
[totPerHistMaxDd, totPerMaxDd, totPerMaxDddate] = drawdown(totPerReturns,'return');
outan.totPerIr  = totPerIr;
outan.totPerSR  = totPerSR;
% bus day
