...
function [grossreturn_i , tcforec_i] = computeOpenRateBasedReturn1(method, i, c, p, s, wgt, loc_rate, usd_rate, DirectionQuote,TC, yrNbDays)
    ...
    case {'method2', 'm2'}
        ...
        for j=1:ncols   
            ...
            if ~isnan(c(i,j)) && ~isnan(c(i-1,j)) && c(i,j)~=0 && c(i-1,j)~=0
                grossreturn_i(1,j) =  s(i-1,j) * wgt(i-1,j) * (power(p(i,j)/p(i-1,j),DirectionQuote) - 1);
                % add carry
                if s(i-1,j)==1
                    LocRateAdjustment = 0.95; USRateAdjustment = 1.05; % penalize lending, increase borrow
                else
